...
def main():
    ...
    for pi in pirange:
        ...
        if args.plot_style < 2:
            for FDR in FDRrange:
                ...
                filename_pre = 'Lbd%.2f_Si1.0_FDR%d_NH%d_ND%d_MOD%d_EPS%.1f_SE%.1f_SHT%.1f_PM%.2f_NR%d' % (args.lbd_scale, FDR, args.num_hyp, 1, args.mod_choice, args.eps, args.sensitivity, args.shift, pi, args.num_runs)
                ...
                if all_filenames == []:
                    ...
                    run_single(args.num_runs, args.num_hyp, 1, mu_gap, args.lbd_scale, pi, args.alpha0, args.mod_choice, FDR, args.eps, args.sensitivity,args.shift,sigma = 1, verbose = False)
                else:
    ...
    print("Now plotting ... ")
    plot_results(args.plot_style, 0, FDRrange, pirange, hyprange, mu_gap, args.lbd_scale, 1, args.num_hyp, args.num_runs, args.mod_choice, args.eps, epsrange, args.sensitivity,args.shift,shiftrange,lbdrange)
            


if __name__ == "__main__":
    parser = argparse.ArgumentParser()
    parser.add_argument('--FDRrange', type=str, default = '6') # choice of algorithms and parameters 1 for SAFFRON, 2 for LORD, 3 for Alpha-investing, 4 for SAFFRON AI, 5 for PAPRIKA AI, 6 for PAPRIKA
    parser.add_argument('--num-runs', type=int, default = 100) # number of independent trials
    parser.add_argument('--num-hyp', type=int, default = 800) # number of hypotheses
    ...
    parser.add_argument('--mod-choice', type=int, default = 4) # 1 for gaussian tests, 2 for beta alternatives, 3 for truncated exponential example, 4 for bernoulli example (*set mu-gap=0 when not doing gaussian tests*) 
